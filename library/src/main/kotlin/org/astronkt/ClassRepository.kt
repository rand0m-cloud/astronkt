...
class ClassRepository(private val classes: MutableMap<DClassId, MutableList<KClass<*>>> = mutableMapOf()) {
    fun classesForDClass(dclassId: DClassId): List<KClass<*>> =
        classes.getOrElse(dclassId) { listOf() }.filter { clazz ->
            if (isServer) {
                ...
                clazz.annotations.any { it.annotationClass == DClassClient::class }
            }
        }
